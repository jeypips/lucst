...
$students_curriculum_datas = $con->getData("SELECT *, ((prelim+midterm+semifinal)/(3)) as final, IFNULL(prelim, '0') prelim, IFNULL(midterm, '0') midterm, IFNULL(semifinal, '0') semifinal FROM students_curriculum_data WHERE enrollment_id = ".$enrollment[0]['id']);
...
/* foreach ($enrollment[0]['students_curriculum_datas'] as $i => $p) {
	
	if ($p == null) $enrollment[0]['students_curriculum_datas'][$i] = "n/a"; // pdf equals null
