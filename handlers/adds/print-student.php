...
$add = $con->getData("SELECT *, DATE_FORMAT(date, '%M %d, %Y') date FROM adds WHERE id = $_POST[id]");

$student = $con->getData("SELECT id, id_number,firstname, lastname, middlename, course, year_level FROM enrollment WHERE id = ".$add[0]['enrollment_id']);
$add[0]['enrollment_id'] = $student[0];
...
foreach ($add[0] as $i => $p) {
	
	if ($p == null) $add[0][$i] = "n/a"; // pdf equals null
	
};

header("Content-Type: application/json");
echo json_encode($add[0]);
